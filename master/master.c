...
#include <linux/sched/signal.h> // 有时也需要这个头文件
...
/*chang edit bind cpus*/
int ec_master_idle_thread_in = 0;
int ec_master_operation_thread_in = 0;
int ec_master_eoe_thread_in = 0;
...
{
    printk("ecrt_master_activate step 1\n");
    ...
    int eoe_was_running;
#endif
    printk("ecrt_master_activate step 2\n");
    ...
    ec_lock_down(&master->domains_lock);
    printk("ecrt_master_activate step 3\n");
    // finish all domains
    domain_offset = 0;
    list_for_each_entry(domain, &master->domains, list) {
    ...
    printk("ecrt_master_activate step 4\n");
    ec_lock_up(&master->domains_lock);
    printk("ecrt_master_activate step 5\n");
    // restart EoE process and master thread with new locking

    ec_master_thread_stop(master);
    printk("ecrt_master_activate step 6\n");
#ifdef EC_EOE
...
#endif
    /*chang edit bind cpus*/
    ec_master_operation_thread_in = 0;
